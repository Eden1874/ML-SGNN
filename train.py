...
from data import CreditLoader_train, dataset_import, CreditLoader_test
from model import GraphNetwork, EmbeddingCredit
...
class ModelTrainer(object):
    def __init__(self,
                 ...
                 gnn_module,
                 data_loader):
        # set encoder and gnn and data_loader
        ...
        # self.module_params = list(self.gnn_module.parameters())
        self.optimizer = optim.Adam(params=self.module_params,
                                    lr=tt.arg.lr,
                                    weight_decay=tt.arg.weight_decay)
        # self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='min',
        #                                                       factor=0.5, patience=100, verbose=True, threshold=0.001,
        #                                                       min_lr=0, eps=1e-08)
    ...
    def train(self):
        ...
        for iter in range(self.global_step + 1, tt.arg.train_iteration + 1):
            ...
            # (1) encode data
            full_data = self.enc_module(full_data)
            ...
            if tt.arg.train_transductive:
                full_logit_layer = self.gnn_module(node_feat=full_data, edge_feat=init_edge)
            else:
            ...
            # (4) compute loss
            full_edge_loss_layer = self.edge_loss((1 - full_logit_layer[:, 0]), (1 - full_edge[:, 0]))
            pos_query_edge_loss_layer = torch.sum(full_edge_loss_layer * query_edge_mask * full_edge[:, 0] * evaluation_mask) / torch.sum(query_edge_mask * full_edge[:, 0] * evaluation_mask)
            neg_query_edge_loss_layer = torch.sum(full_edge_loss_layer * query_edge_mask * (1 - full_edge[:, 0]) * evaluation_mask) / torch.sum(query_edge_mask * (1 - full_edge[:, 0]) * evaluation_mask)
            query_edge_loss_layer = pos_query_edge_loss_layer + neg_query_edge_loss_layer

            # compute accuracy  ------  bian ju zhen yu ce wei zhi de acc
            full_edge_accr_layer = self.hit(full_logit_layer, 1-full_edge[:, 0].long())
            ...
            one_hot_encode = self.one_hot_encode(tt.arg.num_ways_train, support_label.long())
            query_node_pred_layer = torch.bmm(full_logit_layer[:, 0, num_supports:, :num_supports], one_hot_encode) # (num_tasks x num_quries x num_supports) * (num_tasks x num_supports x num_ways)
            query_node_accr_layer = torch.eq(torch.max(query_node_pred_layer, -1)[1], query_label.long()).float().mean()

            # edge_loss_layer.backward()
            self.optimizer.zero_grad()
            query_edge_loss_layer.backward()
            self.optimizer.step()
            # self.scheduler.step(query_edge_loss_layer.item())
            ...
            # logging
            tt.log_scalar('train/edge_loss', query_edge_loss_layer, self.global_step)
    ...
    def one_hot_encode(self, num_classes, class_idx):
        return torch.eye(num_classes).to(tt.arg.device)[class_idx]
...
def set_exp_name():
    exp_name = 'Exp_Name_{}'.format(tt.arg.dataset)
    exp_name += '_{}_way_{}_shot_{}_unlabel'.format(tt.arg.num_ways, tt.arg.num_shots, tt.arg.num_unlabeled)
    exp_name += '_{}_layers'.format(tt.arg.num_layers, tt.arg.meta_batch_size)
    exp_name += '_{}_trans'.format(tt.arg.transductive)
...
if __name__ == '__main__':
    ...
    tt.arg.dataset_root = './'
    tt.arg.dataset = 'credit'
    tt.arg.num_ways = 2
    tt.arg.num_shots = 5
    tt.arg.num_unlabeled = 0
    tt.arg.num_layers = 3
    tt.arg.num_sample = 30
    tt.arg.meta_batch_size = 20
    tt.arg.test_batch_size = 20
    ...
    tt.arg.num_edge_features = 16
    tt.arg.num_node_features = 16
    tt.arg.emb_size = 16

    # train, test parameters
    tt.arg.train_iteration = tt.arg.test_iteration = tt.arg.test_interval = 1000
    tt.arg.log_step = 10
    ...
    tt.arg.experiment = set_exp_name()
    print(tt.arg.experiment)
    ...
    enc_module = EmbeddingCredit(emb_size=tt.arg.emb_size)
    ...
    train_data, test_data = dataset_import(root=tt.arg.dataset_root)
    train_loader = CreditLoader_train(train_data)
    valid_loader = CreditLoader_test(train_data, test_data)
